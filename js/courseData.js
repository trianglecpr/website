var sites = {
  '2733': 'Raleigh',
  '65613': 'Durham'
}
var courseIDs = {
  'bls': {
    'title': 'Basic Life Support for the Healthcare Provider',
    'classes': [
      {
        'id': '7276',
        'title': 'Healthcare Provider CPR (BLS)',
        'cost': 'Class: $60, Required Book $17.25'
      },
      {
        'id': '52177',
        'title': 'Healthcare Provider CPR (BLS) Online and Skills Check',
        'cost': 'Class: $75'
      }]
    },
  'acls': {
    'title': 'Advanced Cardiac Life Support',
    'classes': [
      {
        'id': '7278',
        'title': 'ACLS - Initial',
        'cost': 'Class: $225, Required Book $45.25'
      },
      {
        'id': '7277',
        'title': 'ACLS Renewal',
        'cost': 'Class: $150, Required Book $45.25'
      },
      {
        'id': '7475',
        'title': 'ACLS Online and Skills Check',
        'cost': 'Fee: $220'
      },
      {
        'id': '7280',
        'title': 'ACLS/BLS Initial',
        'cost': 'Class: $265, Required Book $45.25'
      },
      {
        'id': '7279',
        'title': 'ACLS/BLS Renewal',
        'cost': 'Class: $190, Required Book $45.25'
      }]
  },
  'pals': {
    'title': 'Pediatric Advanced Life Support',
    'classes': [
      {
        'id': '7284',
        'title': 'PALS Initial',
        'cost': 'Class: $225, Required Book 52.25'
      },
      {
        'id': '7285',
        'title': 'PALS Renewal',
        'cost': 'Class: $150, Required Book $52.25'
      },
      {
        'id': '7288',
        'title': 'PALS Online and Skills Check',
        'cost': 'Fee: $220'
      }]
    },
  'heartsaver': {
    'title': 'Heartsaver CPR or CPR/First Aid',
    'classes': [
      {
        'id': '7281',
        'title': 'Heartsaver CPR',
        'cost': 'Class: $60'
      },
      {
        'id': '31172',
        'title': 'Heartsaver CPR Classroom Course & Online First Aid Skills Check',
        'cost': 'Fee: $85'
      },
      {
        'id': '7283',
        'title': 'Pediatric First Aid & CPR',
        'cost': 'Class: $85'
      }]
  }
};
